...
public class AngleShooterSubsystem extends SubsystemBase {
	...
	public double calculateSpeakerAngle() {
		...
		desiredShooterAngle = this.adjustAngleForDistance(desiredShooterAngle, offsetSpeakerdist);
	...
	private double adjustAngleForDistance(double angle, double distance) {
		double errorMeters = Math.pow(distance, ADJUSTING_EQUATION_B) + ADJUSTING_EQUATION_D;
		if(errorMeters>0) {
