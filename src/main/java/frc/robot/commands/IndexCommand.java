  ...
  public void execute() {
    ...
    if (!intake.isNoteSeen()) {
      ...
      if(revUpSupplier.getAsBoolean()||stageAngleSup.getAsBoolean()||subwooferAngleSup.getAsBoolean()) {
        if(angleShooterSubsytem.shortSpeakerDist()||subwooferAngleSup.getAsBoolean()||PassSup.getAsBoolean()) {
          if(PassSup.getAsBoolean()) {
            shooter.shootRPS(ShooterConstants.PASSING_RPS);
          } else {
